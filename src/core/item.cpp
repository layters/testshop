...
{}
////////////////////
neroshop::Item::Item(const std::string& id) {//: Item() { // for registered items that already have an id
...
void neroshop::Item::register_item(const std::string& name, const std::string& description, double price, double weight, double length, double width, double height, const std::string& condition, const std::string& product_code) {
    ...
    neroshop::print(std::string("\033[1;36m") + name + " (id: " + get_id() + ") has been registered" + "\033[0m");
#endif
}
////////////////////
...
void neroshop::Item::create_categories_and_subcategories_table(void) {
    ...
    if(!database->table_exists("categories")) {
         ...
         database->execute("ALTER TABLE categories ADD COLUMN IF NOT EXISTS name text;");
         database->execute("ALTER TABLE categories ADD COLUMN IF NOT EXISTS alt_names text DEFAULT null;"); // or alternative_names // store in string array anaa ??
         //database->execute("ALTER TABLE categories ADD COLUMN IF NOT EXISTS desc text DEFAULT null;");
         // category:Pets subcategory:Dog, product:Rottweiler subcategory_parent_id: Dog(id)
         // create table: subcategories
         if(!database->table_exists("subcategories")) {
             database->create_table("subcategories");
             database->execute("ALTER TABLE subcategories ADD COLUMN IF NOT EXISTS name text;");
             database->execute("ALTER TABLE subcategories ADD COLUMN IF NOT EXISTS category_id integer REFERENCES categories(id);"); // or parent_id
             database->execute("ALTER TABLE subcategories ADD COLUMN IF NOT EXISTS description text DEFAULT null;");
             ...
             category_id = database->get_integer("INSERT INTO categories (name, alt_names) VALUES ('?', '') RETURNING id;");
             // subcategories here
             ...
             //---------------------------
             // category here
             category_id = database->get_integer("INSERT INTO categories (name, alt_names) VALUES ('Food', 'Grocery & Gourmet Foods; Produce') RETURNING id;"); // Almost done :) // Food & Beverage in one category? nah
             // subcategories here
...
/*#ifdef __gnu_linux__
    ...
    if(!database->table_exists("images")) {
        ...
        database->add_column("images", "item_id", "integer REFERENCES item(id)");
    	database->add_column("images", "name", "text ARRAY");// include ext ////database->add_column("images", "file", "text");
    	...
    	database->execute("ALTER TABLE images ADD COLUMN IF NOT EXISTS data oid ARRAY;");//"bytea");//"blob");//"lo");
    }
    //////////////////////////
    ...
    database->execute_params("INSERT INTO images(name, data) VALUES($1, bytea_import($2))", { image_name, image_file }); // pg_read_binary_file($2) // pg_read_file($2)::bytea
    // select data
    std::cout << database->get_text("SELECT name, data FROM images;") << std::endl;*/ //;//database->execute_params("", {});
    ///////////////////////////
    ...
    int image_count = database->get_integer_params("SELECT cardinality(data) FROM images WHERE product_id = $1", { std::to_string(this->id) });//("SELECT COUNT(*) FROM images WHERE product_id = $1", { std::to_string(this->id) });//std::cout << "image_count: " << image_count << std::endl; // temp
    ...
    Oid image_oid = lo_creat(database->get_handle(), INV_READ|INV_WRITE);
    if(image_oid == InvalidOid) { std::cout << NEROSHOP_TAG POSTGRESQL_TAG_ERROR "failed to create OID " << image_oid << "\033[0m" << std::endl; database->execute("ROLLBACK;"); return; }
    ...
    int image_fd = lo_open(database->get_handle(), image_oid, INV_WRITE); // write only
    if(image_fd == -1) {
    ...
    if(!image_file_r.good()) {
        std::cout << NEROSHOP_TAG "failed to load " << image_file << std::endl; database->execute("ROLLBACK;"); return;
    }
    ...
    if(lo_write(database->get_handle(), image_fd, reinterpret_cast<const char *>(buffer.data())/*lo_content*/, file_size/*sizeof(lo_content)*/) == -1) {//image_write, (image.get_width() * image.get_height() * image.get_channel())) == -1) {
    ...
    if(lo_close(database->get_handle(), image_fd) == -1) {
    ...
    std::string item_id_found = database->get_text_params("SELECT exists (SELECT item_id FROM images WHERE product_id = $1 LIMIT 1);", { std::to_string(this->id) });
    bool item_id_found_in_table_image = (item_id_found == "t") ? true : false;
    if(item_id_found_in_table_image) { 
        database->execute_params("UPDATE images SET name = array_append(name, $1::text) WHERE item_id = $2", { image_name, std::to_string(this->id) });
        database->execute_params("UPDATE images SET data = array_append(data, $1::oid) WHERE item_id = $2", { std::to_string(image_oid), std::to_string(this->id) });//database->execute_params("UPDATE images SET data = data || '{$1::oid}' WHERE item_id = $2", { std::to_string(image_oid), std::to_string(this->id) });
        neroshop::print("added image upload", 3);
    }
    // check if item_id is already in table image. If not, insert item_id into table image
    if(!item_id_found_in_table_image) { 
        database->execute_params("INSERT INTO images(item_id, name, data) VALUES($1, $2::text[], $3::oid[])", { std::to_string(this->id), "{" + image_name + "}", "{" + std::to_string(image_oid) + "}" }); // or $3::oid[] with "{" + std::to_string(image_oid) + "}"////database->execute_params("INSERT INTO images(item_id, name, data) VALUES($1, $2, $3::oid)", { std::to_string(this->id), image_name, std::to_string(image_oid) });
        neroshop::print("inserted new image data", 2);
    }        
    ///////////////////////////
    // check to see if image data is correct by exporting large object (optional)
    // 
    /*if(lo_export(database->get_handle(), image_oid, ("/tmp/" + image_name + ".png").c_str()) == -1) { // Returns 1 on success, -1 on failure
        std::cout << POSTGRESQL_TAG_ERROR "failed to import file to " << ("/tmp/" + image_name + ".png").c_str() << "\033[0m" << std::endl;
    }*/
    // import the image file
    ////std::cout << lo_import_with_oid(database->get_handle(), image_file.c_str(), image_oid) << std::endl; // 0 InvalidOid (zero)//lo_import(database->get_handle(), image_file.c_str()); // returns InvalidOid (zero)    
    ///////////////////////////
...
/*Image * neroshop::Item::get_upload_image(int index) const {
    ...
    Oid image_oid = database->get_integer_params("SELECT data[$1] FROM images WHERE item_id = $2;", { std::to_string(index), std::to_string(this->id) });//("SELECT data FROM images WHERE product_id = $1;", { std::to_string(this->id) });//std::cout << image_oid << std::endl;
    if(image_oid == InvalidOid) { std::cout << NEROSHOP_TAG POSTGRESQL_TAG_ERROR "OID not found for this particular image (No image(s) uploaded for this item) " << "\033[0m" << std::endl; database->execute("ROLLBACK;"); return nullptr; } // ABORT; is the same as ROLLBACK;
    // get image name (with extension included)
    std::string image_name = database->get_text_params("SELECT name[$1] FROM images WHERE item_id = $2;", { std::to_string(index), std::to_string(this->id) });//std::cout << image_name << std::endl;
    ///////////////////////////
    ////std::cout << lo_import_with_oid(database->get_handle(), "/tmp/random.png", image_oid) << std::endl;
    ...
    if(!std::filesystem::is_directory(NEROSHOP_PATH)) { neroshop::print("directory \"" + std::string(NEROSHOP_PATH) + "\" not found", 1); database->execute("ROLLBACK;"); return nullptr; }
    ...
    if(lo_export(database->get_handle(), image_oid, image_export_path.c_str()) == -1) { // Returns 1 on success, -1 on failure
...
void neroshop::Item::delete_upload_image(int index) {
    ...
    Oid image_oid = database->get_integer_params("SELECT data[$1] FROM images WHERE item_id = $2;", { std::to_string(index), std::to_string(this->id) });//std::cout << image_oid << std::endl;
    if(image_oid == InvalidOid) { database->execute("ROLLBACK;"); return; } // ABORT; is the same as ROLLBACK;
    // delete large object
    int lo_result = lo_unlink(database->get_handle(), image_oid);
    if(lo_result == -1) {
    ...
    database->execute_params("UPDATE images SET name = array_remove(name, name[$1]) WHERE item_id = $2", { std::to_string(index), std::to_string(this->id) });
    database->execute_params("UPDATE images SET data = array_remove(data, data[$1]) WHERE item_id = $2", { std::to_string(index), std::to_string(this->id) });
    ...
    int image_count = database->get_integer_params("SELECT cardinality(data) FROM images WHERE product_id = $1", { std::to_string(this->id) });
    if(image_count == 0) {
        database->execute_params("DELETE FROM images WHERE product_id = $1", { std::to_string(this->id) });
...
#if defined(NEROSHOP_USE_POSTGRESQL)    
    database->execute_params("UPDATE cart_item SET item_qty = $1 WHERE cart_id = $2 AND item_id = $3", { std::to_string(quantity), std::to_string(cart_id), std::to_string(this->id) });
#endif
}
////////////////////
...
#if defined(NEROSHOP_USE_POSTGRESQL)    
    // registered_user cart
    database->execute_params("UPDATE cart_item SET item_qty = $1 WHERE cart_id = $2 AND item_id = $3", { std::to_string(quantity), std::to_string(cart_id), std::to_string(item_id) });
#endif
}
////////////////////
...
////////////////////
////////////////////
std::string neroshop::Item::get_id() const {
    return id;
}
////////////////////
...
#if defined(NEROSHOP_USE_POSTGRESQL)    
    //database->connect("host=127.0.0.1 port=5432 user=postgres password=postgres dbname=neroshoptest");    
    std::string item_name = database->get_text_params("SELECT name FROM item WHERE id = $1", { std::to_string(this->id) });
    //database->finish();
    ////////////////////////////////    
    return item_name;
#endif
...
#if defined(NEROSHOP_USE_POSTGRESQL)    
    //database->connect("host=127.0.0.1 port=5432 user=postgres password=postgres dbname=neroshoptest");    
    std::string item_name = database->get_text_params("SELECT name FROM item WHERE id = $1", { std::to_string(item_id) });
    //database->finish();
    ////////////////////////////////
    return item_name;     
#endif
...
#if defined(NEROSHOP_USE_POSTGRESQL)    
    //database->connect("host=127.0.0.1 port=5432 user=postgres password=postgres dbname=neroshoptest");    
    double item_price = database->get_real_params("SELECT price FROM item WHERE id = $1", { std::to_string(this->id) });
    //database->finish();
    ////////////////////////////////    
    return item_price;
#endif    
...
#if defined(NEROSHOP_USE_POSTGRESQL)    
    //database->connect("host=127.0.0.1 port=5432 user=postgres password=postgres dbname=neroshoptest");    
    double item_price = database->get_real_params("SELECT price FROM item WHERE id = $1", { std::to_string(item_id) });
    //database->finish();
    ////////////////////////////////    
    return item_price;
#endif    
...
unsigned int neroshop::Item::get_quantity(unsigned int cart_id) const {
    ...
    // registered_user cart    
#if defined(NEROSHOP_USE_POSTGRESQL)    
    unsigned int quantity = database->get_integer_params("SELECT item_qty FROM cart_item WHERE cart_id = $1 AND item_id = $2", { std::to_string(cart_id), std::to_string(this->id) });
    return quantity;
#endif
    return 0;    
}
...
unsigned int neroshop::Item::get_quantity(unsigned int item_id, unsigned int cart_id) {
    ...
    // registered_user cart
#if defined(NEROSHOP_USE_POSTGRESQL)    
    unsigned int quantity = database->get_integer_params("SELECT item_qty FROM cart_item WHERE cart_id = $1 AND item_id = $2", { std::to_string(cart_id), std::to_string(item_id) });
    return quantity;
#endif    
    return 0;
}
...
#if defined(NEROSHOP_USE_POSTGRESQL)    
    //database->connect("host=127.0.0.1 port=5432 user=postgres password=postgres dbname=neroshoptest");    
    double item_weight = database->get_real_params("SELECT weight FROM item WHERE id = $1", { std::to_string(this->id) });
    //database->finish();
    ////////////////////////////////    
    return item_weight;
#endif
...
#if defined(NEROSHOP_USE_POSTGRESQL)    
    //database->connect("host=127.0.0.1 port=5432 user=postgres password=postgres dbname=neroshoptest");    
    double item_weight = database->get_real_params("SELECT weight FROM item WHERE id = $1", { std::to_string(item_id) });
    //database->finish();
    ////////////////////////////////    
    return item_weight;
#endif
...
#if defined(NEROSHOP_USE_POSTGRESQL)
    //database->connect("host=127.0.0.1 port=5432 user=postgres password=postgres dbname=neroshoptest");	
    if(!database->table_exists("product_ratings")) {/*//database->finish();*/ return 0;}
    unsigned int ratings_count = database->get_integer_params("SELECT COUNT(*) FROM product_ratings WHERE product_id = $1", { std::to_string(this->id) });    
	//database->finish();
...
int neroshop::Item::get_ratings_count(unsigned int item_id) {
    ...
    //database->connect("host=127.0.0.1 port=5432 user=postgres password=postgres dbname=neroshoptest");	
    if(!database->table_exists("product_ratings")) {/*//database->finish();*/ return 0;}
    unsigned int ratings_count = database->get_integer_params("SELECT COUNT(*) FROM product_ratings WHERE product_id = $1", { std::to_string(item_id) });    
	//database->finish();
    return ratings_count;
#endif
    return 0;  
...
int neroshop::Item::get_star_count(int star_number) {
    ...
    //database->connect("host=127.0.0.1 port=5432 user=postgres password=postgres dbname=neroshoptest");
    if(!database->table_exists("product_ratings")) { return 0; }
    unsigned int star_count = database->get_integer_params("SELECT COUNT(stars) FROM product_ratings WHERE product_id = $1 AND stars = $2", { std::to_string(this->id), std::to_string(star_number) });
    //database->finish();
    return star_count;
#endif
    return 0;    
...
int neroshop::Item::get_star_count(unsigned int item_id, int star_number) {
    ...
    //database->connect("host=127.0.0.1 port=5432 user=postgres password=postgres dbname=neroshoptest");
    if(!database->table_exists("product_ratings")) { return 0; }
    unsigned int star_count = database->get_integer_params("SELECT COUNT(stars) FROM product_ratings WHERE product_id = $1 AND stars = $2", { std::to_string(item_id), std::to_string(star_number) });
    //database->finish();
    return star_count;
#endif
    return 0;    
...
#if defined(NEROSHOP_USE_POSTGRESQL)
    //database->connect("host=127.0.0.1 port=5432 user=postgres password=postgres dbname=neroshoptest");
    if(!database->table_exists("product_ratings")) { return 0.0f; }
    // get number of star ratings (for a specific item_id)
    unsigned int total_star_ratings = database->get_integer_params("SELECT COUNT(*) FROM product_ratings WHERE product_id = $1", { std::to_string(this->id) });
    if(total_star_ratings == 0) { /*std::cout << "This item has no star ratings\n";*/ return 0.0f; }
    // get number of 1, 2, 3, 4, and 5 star_ratings
    int one_star_count = database->get_integer_params("SELECT COUNT(stars) FROM product_ratings WHERE product_id = $1 AND stars = $2", { std::to_string(this->id), std::to_string(1) });
    int two_star_count = database->get_integer_params("SELECT COUNT(stars) FROM product_ratings WHERE product_id = $1 AND stars = $2", { std::to_string(this->id), std::to_string(2) });
    int three_star_count = database->get_integer_params("SELECT COUNT(stars) FROM product_ratings WHERE product_id = $1 AND stars = $2", { std::to_string(this->id), std::to_string(3) });
    int four_star_count = database->get_integer_params("SELECT COUNT(stars) FROM product_ratings WHERE product_id = $1 AND stars = $2", { std::to_string(this->id), std::to_string(4) });
    int five_star_count = database->get_integer_params("SELECT COUNT(stars) FROM product_ratings WHERE product_id = $1 AND stars = $2", { std::to_string(this->id), std::to_string(5) });
...
float neroshop::Item::get_average_stars(unsigned int item_id) {
    ...
    if(!database->table_exists("product_ratings")) { return 0.0f; }
    // get number of star ratings (for a specific item_id)
    unsigned int total_star_ratings = database->get_integer_params("SELECT COUNT(*) FROM product_ratings WHERE product_id = $1", { std::to_string(item_id) });
    if(total_star_ratings == 0) { /*std::cout << "This item has no star ratings\n";*/ return 0.0f; }
    // get number of 1, 2, 3, 4, and 5 star_ratings
    int one_star_count = database->get_integer_params("SELECT COUNT(stars) FROM product_ratings WHERE product_id = $1 AND stars = $2", { std::to_string(item_id), std::to_string(1) });
    int two_star_count = database->get_integer_params("SELECT COUNT(stars) FROM product_ratings WHERE product_id = $1 AND stars = $2", { std::to_string(item_id), std::to_string(2) });
    int three_star_count = database->get_integer_params("SELECT COUNT(stars) FROM product_ratings WHERE product_id = $1 AND stars = $2", { std::to_string(item_id), std::to_string(3) });
    int four_star_count = database->get_integer_params("SELECT COUNT(stars) FROM product_ratings WHERE product_id = $1 AND stars = $2", { std::to_string(item_id), std::to_string(4) });
    int five_star_count = database->get_integer_params("SELECT COUNT(stars) FROM product_ratings WHERE product_id = $1 AND stars = $2", { std::to_string(item_id), std::to_string(5) });
    // now calculate average stars
    // 3 total star ratings:(5, 3, 4) // average = (5 + 3 + 4) / 3 = 4 stars        
    float average_stars = (
        (1 * static_cast<float>(one_star_count)) + 
...
#if defined(NEROSHOP_USE_POSTGRESQL)    
    //database->connect("host=127.0.0.1 port=5432 user=postgres password=postgres dbname=neroshoptest");    
    int stock_qty = database->get_integer_params("SELECT stock_qty FROM inventory WHERE product_id = $1 AND stock_qty > 0", { std::to_string(this->id) });
    //database->finish();
    ////////////////////////////////    
    return stock_qty;
#endif
...
#if defined(NEROSHOP_USE_POSTGRESQL)    
    //database->connect("host=127.0.0.1 port=5432 user=postgres password=postgres dbname=neroshoptest");    
    int stock_qty = database->get_integer_params("SELECT stock_qty FROM inventory WHERE product_id = $1 AND stock_qty > 0", { std::to_string(item_id) });
    //database->finish();
    ////////////////////////////////
    return stock_qty;
#endif
...
#if defined(NEROSHOP_USE_POSTGRESQL)
    int seller_count = database->get_integer_params("SELECT COUNT(seller_id) FROM inventory WHERE product_id = $1;", { std::to_string(this->id) });
    return seller_count;
#endif
    return 0;    
}
...
#if defined(NEROSHOP_USE_POSTGRESQL)
    int seller_count = database->get_integer_params("SELECT COUNT(seller_id) FROM inventory WHERE product_id = $1;", { std::to_string(item_id) });
    return seller_count;
#endif
    return 0;    
}
...
unsigned int neroshop::Item::get_seller_id() const { // might need a little more work
    ...
    int seller_id = database->get_integer_params("SELECT inventory.seller_id FROM inventory JOIN seller_ratings ON inventory.seller_id = seller_ratings.seller_id WHERE inventory.product_id = $1 AND score = 1 GROUP BY inventory.seller_id ORDER BY COUNT(score) DESC LIMIT 1;", { std::to_string(this->id) });//int seller_id = database->get_integer_params("SELECT seller_id FROM inventory WHERE product_id = $1 AND stock_qty > 0;", { std::to_string(this->id) });
    ...
    if(seller_id == 0) seller_id = database->get_integer_params("SELECT seller_id FROM inventory WHERE product_id = $1;", { std::to_string(this->id) });//int seller_id = database->get_integer_params("SELECT seller_id FROM inventory WHERE product_id = $1 AND stock_qty > 0;", { std::to_string(this->id) });
    return seller_id;
#endif
    return 0;    
}
...
#if defined(NEROSHOP_USE_POSTGRESQL)    
    // prioritize the seller with the most positive (or good) ratings that sells this item
    int seller_id = database->get_integer_params("SELECT inventory.seller_id FROM inventory JOIN seller_ratings ON inventory.seller_id = seller_ratings.seller_id WHERE inventory.product_id = $1 AND score = 1 GROUP BY inventory.seller_id ORDER BY COUNT(score) DESC LIMIT 1;", { std::to_string(item_id) });//int seller_id = database->get_integer_params("SELECT seller_id FROM inventory WHERE product_id = $1 AND stock_qty > 0;", { std::to_string(this->id) });
    // if the seller_ratings table does not exist yet or the seller has not yet been rated then we can just get any random seller (or maybe I should get the first seller to list this item??)
    if(seller_id == 0) seller_id = database->get_integer_params("SELECT seller_id FROM inventory WHERE product_id = $1;", { std::to_string(item_id) });//int seller_id = database->get_integer_params("SELECT seller_id FROM inventory WHERE product_id = $1 AND stock_qty > 0;", { std::to_string(item_id) });
    return seller_id;
#endif
    return 0;    
}
...
std::vector<unsigned int> neroshop::Item::get_seller_ids() const {
    ...
    std::string command = "SELECT DISTINCT seller_id FROM inventory WHERE product_id = $1";// AND stock_qty > 0;";
...
#if defined(NEROSHOP_USE_POSTGRESQL)    
    //database->connect("host=127.0.0.1 port=5432 user=postgres password=postgres dbname=neroshoptest");    
    int id = database->get_integer_params("SELECT id FROM item WHERE id = $1", { std::to_string(this->id) });
    //database->finish();
    ////////////////////////////////
    return (id > 0);
#endif
...
#if defined(NEROSHOP_USE_POSTGRESQL)    
    //database->connect("host=127.0.0.1 port=5432 user=postgres password=postgres dbname=neroshoptest");    
    int id = database->get_integer_params("SELECT id FROM item WHERE id = $1", { std::to_string(item_id) });
    //database->finish();
    ////////////////////////////////    
    return (id > 0);
#endif
...
#if defined(NEROSHOP_USE_POSTGRESQL)    
    //database->connect("host=127.0.0.1 port=5432 user=postgres password=postgres dbname=neroshoptest");    
    int stock_qty = database->get_integer_params("SELECT stock_qty FROM inventory WHERE product_id = $1 AND stock_qty > 0", { std::to_string(this->id) }); // check stock_qty regardless of who is selling said item
    //database->finish();
    ////////////////////////////////    
    return (stock_qty != 0);
#endif
...
#if defined(NEROSHOP_USE_POSTGRESQL)    
    //database->connect("host=127.0.0.1 port=5432 user=postgres password=postgres dbname=neroshoptest");    
    int stock_qty = database->get_integer_params("SELECT stock_qty FROM inventory WHERE product_id = $1 AND stock_qty > 0", { std::to_string(item_id) }); // check stock_qty regardless of who is selling said item
