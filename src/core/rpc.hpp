...
namespace rpc {
    static std::vector<std::string> methods = {"query"};//static std::unordered_map<std::string, std::function<void(int)>> methods_cmd
    static std::vector<std::string> query_methods = { 
    ...
    static/*extern*/ std::string translate_cmd(const std::string& method, Args&&... args) { // converts a method (string) and its argument(s) into a json_rpc response message via the command line
